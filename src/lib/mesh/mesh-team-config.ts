/**
 * 团队配置 —— 单例（一个 team）。Leader 命令经 Control Plane 改这里，协作据此调整。
...
export function getTeamConfig(): TeamConfig {
  const row = getDb()
    .prepare('SELECT blacklist_json, focus, mode FROM mesh_team_config WHERE id = ?')
    .get(DEFAULT_ID) as { blacklist_json: string; focus: string; mode: TeamMode } | undefined
  if (!row) return { ...DEFAULTS }
  return { blacklist: safeArr(row.blacklist_json), focus: row.focus, mode: row.mode }
}

export function upsertTeamConfig(patch: Partial<TeamConfig>): TeamConfig {
  const cur = getTeamConfig()
  ...
  getDb()
    .prepare(
      `INSERT INTO mesh_team_config (id, blacklist_json, focus, mode, updated_at)
       VALUES (?, ?, ?, ?, datetime('now'))
       ON CONFLICT(id) DO UPDATE SET
         blacklist_json=excluded.blacklist_json, focus=excluded.focus,
         mode=excluded.mode, updated_at=datetime('now')`,
    )
    .run(DEFAULT_ID, JSON.stringify(next.blacklist), next.focus, next.mode)
  return next
}
