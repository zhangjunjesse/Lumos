...
const REMIX_ANALYZE = [
  ...
  '  },',
  '  "style_retention": {  // Step4 — the CONCRETE style worth keeping (only matters when staying close in style) + drift words to AVOID',
  '    "emotion":"the emotional tone to keep",',
  '    "texture":"concrete texture words, e.g. soft colored-pencil texture / chalky crayon edges / halftone grain / clean flat vector",',
  '    "linework":"line character to keep, e.g. gentle rounded lines / thin clean line art / distressed broken strokes",',
  '    "color_mood":"palette mood to keep, e.g. low-contrast pastel / warm vintage / bold high-saturation",',
  '    "lettering":"lettering character to keep, e.g. gentle rounded hand-lettering / bold sans / none",',
  '    "avoid":"style-drift words that would betray this style, e.g. thick dark outline, screen print, badge-like, bold commercial vintage, hard inked graphic, high contrast"',
  '  },',
  '  "risk": {  // Step6 — imitation-risk assessment',
  '    "source_type":"ai|hand-drawn|photo|vector|unsure",',
  '    "unique_composition_risk":"a distinctive, recognizable composition/layout that must NOT be copied — else \\"\\"",',
  '    "unique_text_risk":"a distinctive wording combination that must NOT be reused — else \\"\\"",',
  '    "safe_transfer":"what is SAFE to carry over (emotion / color energy / theme semantics / audience / big direction)",',
  '    "must_replace":"what MUST be changed (subject details / pose / composition / symbol combo / text / layout rhythm / character traits)",',
  '    "target_similarity":"low|mid-low|mid|high",  // non-owned reference → low or mid-low',
  '    "strategy":"low-similarity commercial remix|mid-low style translation|high-similarity owned-image polish"',
  '  },',
  '  "niches": [ 3-5 of {"buyer":"","use_case":"","emotion":"","visual_theme":"","search_intent":"","search_terms":"direction only, not final SEO","match":"high|mid|low","potential":"high|mid|low","risk":"","verified":"yes|no — yes only if backed by the provided VERIFIED MARKET DATA, else no (inference)"} ],',
  ...
  '  "palette_constraints": {  // Step7 — SOLVE these constraints first, then derive the palettes below',
  '    "shirt_color":"black|white|cream|dark|light|unsure", "thumbnail_contrast":"high|mid|low", "print_complexity":"1-color|2-3 colors|4-5 colors|many",',
  '    "keep_colors":"original colors worth keeping", "replace_colors":"original colors that must be replaced", "target_mood":"",',
  '    "target_audience":"kids|women|men|unisex|profession|faith|pet-owner|other", "season":"season/holiday constraint, else \\"\\"", "dual_shirt":"yes|no"',
  '  },',
  ...
  '  "sticker_check": {  // Step8 — printability / stickerizability check (each yes|no)',
  '    "remove_bg":"yes|no", "clear_outline":"yes|no", "single_focal":"yes|no", "can_simplify":"yes|no",',
  '    "transparent_ok":"yes|no", "readable_thumbnail":"yes|no", "can_series":"yes|no"',
  '  }',
  '}',
...
// {direction}方向矩阵+相似度目标 / {styleRetention}风格保留目标(Step4) / {hook}创意钩子 / {palette}配色方案 /
// {paletteConstraints}配色约束(Step7) / {title} / {textRule} / {riskRule}仿图风险约束含IP剔除(Step6)。代码逐张注入。
const REMIX_VARIANT = [
  ...
  '{hook}',
  '',
  'Color constraints (Step7 — respect these):',
  '{paletteConstraints}',
  ...
  '{textRule}',
  '',
  'Imitation-risk constraints (Step6 — remove IP, replace risky elements):',
  '{riskRule}',
  '',
