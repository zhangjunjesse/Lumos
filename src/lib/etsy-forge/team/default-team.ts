...
export const DEFAULT_TEAM_SOP = [
  ...
  '',
  '## 流程',
  '1. 派策划:把创作简报**原文照抄**进任务(成员看不到简报,转述会丢信息),要 {N} 条一句话创作指令——整体一半贴近参考印花(保留卖点),一半大胆发散;简报里标注 IP 风险或非自有原图时全部发散,禁止出现「复刻/临摹原图」。',
  '2. 【并行,硬性要求】把 {N} 条指令一次性拆成 {N} 个 Task 同时派给设计师,不许派一张等一张(单张出图要一两分钟,串行会撞会话超时)。每个任务附:指令原文、贴近/发散标记、参考印花路径(仅贴近)、风险规则原文;指令含文字内容时用引号逐字给出并注明「照抄,禁止改写/翻译/纠拼写」。',
  '3. 收齐产出后,把全部图片路径一次性派给审核:按「白底残留/乱码错字/糊/难贴纸化/撞知名IP」五项评 good/weak,weak 给一句原因。',
  '',
  '## 关键决策',
  '- 某条指令出图失败:最多重派一次(任务里写明上次的具体错误);配额被拒立即停手,用已有产出交差。',
  '- 某个成员失败不影响其他任务;哪怕只有一张成功也如实交差。',
  '- 全部失败:不编造,交空 designs 并在 summary 里写清原因。',
  '- 交差的 designs[].path 逐字复制设计师交回的 PATH 行——不要用审核转述的路径,更不要自己拼(路径有真实性硬校验,对不上会被丢弃)。',
].join('\n');
