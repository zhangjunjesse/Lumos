...
export function emitRuntimeHelpers(): string[] {
  return [
    ...
    '  }',
    "  if (ref === 'state') return state;",
    "  if (typeof ref === 'string' && ref.startsWith('state.')) {",
    "    if (state === undefined || state === null) return undefined;",
    "    return __getByPath(state, ref.slice('state.'.length).split('.'));",
    '  }',
    ...
    "    if (value === 'input' || value.startsWith('input.') || value.startsWith('steps.') || value === 'state' || value.startsWith('state.')) {",
    '      return __resolveRef(value, input, stepOutputs, state);',
    '    }',
    "    if (value.includes('{{')) {",
    "      return value.replace(/\\{\\{([^}]+)\\}\\}/g, (_, ref) => {",
    '        const resolved = __resolveRef(ref.trim(), input, stepOutputs, state);',
