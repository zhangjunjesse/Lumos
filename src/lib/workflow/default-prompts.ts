...
ctx.browser.click(selector)         // 点击元素
ctx.browser.fill(selector, value)   // 填充输入框
ctx.browser.type(text)              // 键盘输入
ctx.browser.press(key)              // 按键（如 "Enter"）
ctx.browser.waitFor(selector, { timeout })  // 等待元素出现
...
\`\`\`javascript
// 示例：浏览器自动化
await ctx.browser.navigate('https://example.com/data');
await ctx.browser.waitFor('#data-table');
const data = await ctx.browser.evaluate('JSON.stringify([...document.querySelectorAll("tr")].map(r => r.textContent))');

return {
  success: true,
  output: { summary: '采集到数据: ' + data },
};
\`\`\`
