...
export const WORKFLOW_CODIFY_PROMPT = `你是 Lumos 工作流代码固化助手。你的任务是将 Agent 步骤的执行记录（tool calls 追踪）转换为等效的 TypeScript 代码处理器。
...
3. 可用的 API（如 browser-bridge-client）

## 输出格式
输出一个完整的 TypeScript 代码块，符合 CodeHandler 接口：

\`\`\`typescript
import type { CodeHandler } from '@/lib/workflow/code-handler-types';

const handler: CodeHandler = {
  id: '<kebab-case 唯一ID>',
  name: '<显示名称>',
  description: '<描述>',
  execute: async (ctx) => {
...
- 浏览器操作使用 browser-bridge-client API（fetch HTTP 调用 bridge-server）
