...
export function CodifyAgentSection() {
  const [config, setConfig] = useState<CodifyConfig | null>(null);
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch('/api/workflow/codify-config');
      const data = await res.json() as CodifyConfig;
      setConfig(data);
        ...
        <div className="flex flex-col gap-4 rounded-lg border border-border/60 p-4">
          ...
          <div className="space-y-1.5">
            ...
            <p className="text-xs text-muted-foreground">
              指导 AI 如何从执行追踪生成 TypeScript 代码处理器。
            </p>
            <Textarea
              value={prompt}
