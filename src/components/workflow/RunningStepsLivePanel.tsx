...
export function RunningStepsLivePanel({
  ...
  const entries = Object.entries(stepLiveTraces).filter(
    ([stepId]) => (stepOverlays[stepId]?.status ?? 'running') === 'running',
  );
  if (entries.length === 0) {
    return (
  ...
  return (
    <div className="space-y-3 mb-4">
      {entries.map(([stepId, events]) => {
        const stepLabel = workflowDsl?.nodes.find((n) => n.id === stepId)?.metadata?.label || stepId;
        return (
          <div key={stepId} className="rounded-xl border border-blue-500/30 bg-blue-500/5 p-4">
            ...
            <StepTraceStreamSection events={events} isRunning title="实时输出" />
          </div>
        );
      })}
    </div>
