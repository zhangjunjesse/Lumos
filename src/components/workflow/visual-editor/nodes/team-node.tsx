...
function TeamNodeInner({ data, selected }: NodeProps & { data: StepNodeData }) {
  ...
  const input = (data.node.type === 'team' ? data.node.input : {}) as Record<string, unknown>;
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-1.5">
        ...
        <span className="text-[11px] font-semibold text-foreground truncate flex-1">团队</span>
        <RunDurationLabel overlay={overlay} />
      </div>
      {task && (
        <div className="mt-1 text-[9px] text-muted-foreground leading-snug line-clamp-1 truncate">{task}</div>
      )}
...
export const TeamNode = memo(TeamNodeInner);
