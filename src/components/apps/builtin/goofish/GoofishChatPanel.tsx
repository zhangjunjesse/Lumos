...
const STORAGE_KEY = 'lumos:goofish-assistant-chat-session';

interface GoofishChatPanelProps {
...
export function GoofishChatPanel({
  compactInputOnly = false,
  onInputFocus,
  onUpdated,
  fullWidth = false,
  hideEmptyState = false,
}: GoofishChatPanelProps): React.ReactElement | null {
  ...
  const loadMessages = React.useCallback(async (id: string, signal?: AbortSignal) => {
    try {
      ...
      // swallow other errors silently — polling is best-effort
    }
  ...
  React.useEffect(() => {
    ...
    async function init() {
      ...
      try {
        const res = await fetch('/api/apps/builtin/goofish/chat/session', {
          method: 'POST',
        ...
        if (!res.ok) {
          ...
          throw new Error(body.error || '初始化闲鱼助手会话失败');
        }
        ...
        if (!cancelled) {
          setError(err instanceof Error ? err.message : '初始化闲鱼助手会话失败');
          setLoading(false);
        }
        return;
      }
  ...
  React.useEffect(() => {
    ...
    const interval = window.setInterval(() => {
      void loadMessages(sessionId, ctrl.signal);
    }, 4000);
    return () => {
