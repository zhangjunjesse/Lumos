...
import {
  ECOMMERCE_STARTER_SUGGESTIONS,
  type EcommerceStarterSuggestion,
} from './ecommerce-starter-suggestions';

const STORAGE_KEY = 'lumos:ecommerce-assistant-chat-session';
const CHAT_DRAFT_EVENT = 'lumos:chat-draft';

function dispatchChatDraft(text: string): void {
  if (typeof window === 'undefined') return;
  window.dispatchEvent(
    new CustomEvent(CHAT_DRAFT_EVENT, { detail: { text, mode: 'replace' } }),
  );
}

interface EcommerceChatPanelProps {
...
export function EcommerceChatPanel({
  compactInputOnly = false,
  onInputFocus,
  onUpdated,
  fullWidth = false,
  hideEmptyState = false,
}: EcommerceChatPanelProps): React.ReactElement | null {
  ...
  React.useEffect(() => {
    ...
    async function init() {
      ...
      try {
        const res = await fetch('/api/apps/builtin/ecommerce/chat/session', {
          method: 'POST',
        ...
        if (!res.ok) {
          ...
          throw new Error(body.error || '初始化电商助手会话失败');
        }
        ...
        if (!cancelled) {
          setError(err instanceof Error ? err.message : '初始化电商助手会话失败');
          setLoading(false);
        }
        return;
      }
  ...
  const showStarters = !compactInputOnly && messages.length === 0;
  const handleStarterClick = (suggestion: EcommerceStarterSuggestion) => {
    onInputFocus?.();
    dispatchChatDraft(suggestion.prompt);
  };
  ...
  return (
    <div className="flex h-full min-h-0 flex-col">
      {showStarters ? (
        <div
          data-testid="ecommerce-starter-row"
          className="flex flex-wrap gap-2 border-b border-border/50 px-4 pb-2 pt-1"
        >
          {ECOMMERCE_STARTER_SUGGESTIONS.map((suggestion) => (
            <button
              key={suggestion.id}
              type="button"
              onClick={() => handleStarterClick(suggestion)}
              className="rounded-full border border-border/70 bg-muted/40 px-3 py-1 text-xs font-medium text-muted-foreground transition-colors hover:border-foreground/40 hover:bg-muted hover:text-foreground"
            >
              {suggestion.label}
            </button>
          ))}
