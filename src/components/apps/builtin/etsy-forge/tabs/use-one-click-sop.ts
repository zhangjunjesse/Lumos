...
export function useOneClickSop() {
  ...
  const startSop = async (productIds: string[]): Promise<string | null> => {
    if (productIds.length === 0) return null;
    if (
      !confirm(
        `对 ${productIds.length} 个选中商品「一键出品」？逐商品走 7 步(采集详情→评论分析→图片分类→抠印花→素材+姿势→二创→出产品图)，多商品按「设置→图片生成并发度」并行(采集详情会自动排队避免抢浏览器)。后台跑，进度去右下角「任务」按钮看、失败可单步重试。`,
      )
    )
      return null;
    setSopStarting(true);
    try {
      await etsyForgeApi.startSop(productIds);
