  ...
  return (
    <div>
        ...
        <div className="min-w-0 flex-1">
          {tab === 'basic' && (
            <Section title="基本信息">
              ...
              <Field label={`描述 ${TODO}`}>
                <textarea className={INPUT} rows={2} defaultValue={workshop.desc} />
              </Field>
              <Field label="关注重点（focus，注入盯盘 · 已接存储）">
          ...
          {tab === 'risk' && (
            <Section title="风控规则">
              ...
              <div className="grid grid-cols-2 gap-4 opacity-60">
                <Field label={`单笔最大金额 ${TODO}`}>
                  <input className={INPUT} defaultValue="50000" disabled />
                </Field>
                <Field label={`单日最大亏损 ${TODO}`}>
                  <input className={INPUT} defaultValue="20000" disabled />
                </Field>
              </div>
            </Section>
          )}
      ...
      <div className="mt-8 flex items-center gap-3 border-t border-neutral-200 pt-4">
        ...
        {saved && <span className="text-sm text-emerald-600">已保存（模式/关注/黑名单）</span>}
      </div>
    </div>
  )
}
