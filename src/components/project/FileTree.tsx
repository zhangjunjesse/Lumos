...
function getFileIcon(extension?: string): ReactNode {
  switch (extension) {
    ...
    case "php":
    case "zig":
      return <HugeiconsIcon icon={SquareCode} className="size-4 text-muted-foreground" />;
    case "json":
    case "yaml":
    case "yml":
    case "toml":
      return <HugeiconsIcon icon={Code} className="size-4 text-muted-foreground" />;
    case "md":
    case "mdx":
    case "txt":
    case "csv":
      return <HugeiconsIcon icon={File} className="size-4 text-muted-foreground" />;
    default:
      return <HugeiconsIcon icon={File} className="size-4 text-muted-foreground" />;
  }
}
  ...
  const fetchTree = useCallback(async () => {
    ...
    try {
      const res = await fetch(
        `/api/files?dir=${encodeURIComponent(workingDirectory)}&baseDir=${encodeURIComponent(workingDirectory)}&depth=8&_t=${Date.now()}`
      );
      if (res.ok) {
    ...
    <div className="flex flex-col h-full min-h-0">
      ...
      <div className="flex items-center gap-1.5 px-4 py-2 shrink-0">
        <div className="relative flex-1 min-w-0">
          <HugeiconsIcon icon={Search} className="absolute left-2.5 top-1/2 h-3 w-3 -translate-y-1/2 text-muted-foreground pointer-events-none" />
          <Input
        ...
        <Button
          ...
          className="h-7 w-7 shrink-0"
        >
          <HugeiconsIcon icon={Refresh} className={cn("h-3 w-3", loading && "animate-spin")} />
      ...
      <div className="flex-1 overflow-auto">
        {loading && tree.length === 0 ? (
          <div className="flex items-center justify-center py-8">
            <HugeiconsIcon icon={Refresh} className="h-4 w-4 animate-spin text-muted-foreground" />
          </div>
        ) : tree.length === 0 ? (
