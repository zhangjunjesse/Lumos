...
export async function POST(req: NextRequest) {
  try {
    const body = (await req.json()) as { name?: string; description?: string; sop?: string; members?: unknown; images_per_run?: number; provider_id?: string; model?: string };
    const store = getEtsyForgeStore();
    const userId = getStorageUserId(req);
    const team = createTeam(store, userId, { name: body.name ?? '', description: body.description, sop: body.sop, members: body.members, images_per_run: body.images_per_run, provider_id: body.provider_id, model: body.model });
...
export async function PATCH(req: NextRequest) {
  try {
    const body = (await req.json()) as { id?: string; name?: string; description?: string; sop?: string; members?: unknown; images_per_run?: number; is_default?: boolean; provider_id?: string; model?: string };
