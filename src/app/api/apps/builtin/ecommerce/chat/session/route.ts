...
export async function POST(request: NextRequest) {
  try {
    ...
    const systemPrompt = buildEcommerceAssistantChatSystemPrompt();
    ...
    if (requestedSessionId) {
      ...
      if (existing && isEcommerceAssistantChatSession(existing)) {
    ...
    const message = error instanceof Error ? error.message : '创建电商助手会话失败';
