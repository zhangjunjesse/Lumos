...
export default function WorkflowDetailPage() {
  ...
  useEffect(() => {
    fetch(`/api/workflow/definitions/${id}`)
      .then(r => r.json())
      .then((data: { workflow?: { workflowDsl: unknown; name: string }; error?: string }) => {
        if (data.error || !data.workflow) { setLoadError(data.error ?? '工作流不存在'); return; }
        const loadedDsl = parseLoadedDsl(data.workflow.workflowDsl);
        const text = JSON.stringify(loadedDsl, null, 2);
        setDsl(loadedDsl);
        setDslText(text);
        ...
        applyDirtyState('load-workflow', false, {
          ...
          nextDslText: text,
          extra: { nodeCount: loadedDsl.nodes.length },
        });
      })
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      ...
      const persistedDsl = data.workflow?.workflowDsl
        ? parseLoadedDsl(data.workflow.workflowDsl)
        : dsl;
      ...
      applyDirtyState('save-success', false, {
        ...
        extra: {
          workflowId: id,
          nodeCount: persistedDsl.nodes.length,
      ...
      logWorkflowEditorDebug('save-failed', { workflowId: id });
      setSaveMsg('保存失败');
    }
  ...
  const handleDslChange = useCallback((nextDsl: WorkflowDSLV3, nextText: string) => {
    setDsl(nextDsl);
    setDslText(nextText);
    setValidation(null);
    applyDirtyState('dsl-change', computeDirty(name, nextText), {
      nextName: name,
      nextDslText: nextText,
      extra: { nodeCount: nextDsl.nodes.length },
    });
  ...
  const handleDslTextEdit = useCallback((text: string) => {
    ...
    try {
      ...
      const nextName = typeof nextDsl.name === 'string' ? nextDsl.name : name;
      setDsl(nextDsl);
      if (typeof nextDsl.name === 'string') setName(nextDsl.name);
      setValidation(null);
      applyDirtyState('dsl-text-edit', computeDirty(nextName, text), {
        nextName,
        nextDslText: text,
        extra: { parseSucceeded: true, nodeCount: nextDsl.nodes.length },
      });
    } catch {
      applyDirtyState('dsl-text-edit', computeDirty(name, text), {
        nextName: name,
        nextDslText: text,
        extra: { parseSucceeded: false },
      });
    }
  }, [applyDirtyState, computeDirty, name]);
  ...
  const handleNameBlur = useCallback(() => {
    ...
    if (!dsl || name === dsl.name) return;
    const nextDsl: WorkflowDSLV3 = { ...dsl, name };
  ...
  const handleApplyDsl = useCallback((raw: Record<string, unknown>) => {
    try {
      const nextDsl = parseLoadedDsl(raw);
      const nextText = JSON.stringify(nextDsl, null, 2);
      const nextName = nextDsl.name || name;
      setDsl(nextDsl);
      setDslText(nextText);
      if (nextDsl.name) setName(nextDsl.name);
