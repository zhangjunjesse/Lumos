...
- **团队选择在输入框工具栏一行**(与模型选择器并排,用户验收定稿):空会话是下拉
  (不用团队/团队列表),发出第一条消息后锁定为徽标不可换队。所有聊天入口
  (侧边栏工作区直建会话、/chat 新会话页)共用 MessageInput 内嵌控件。
- 过程可视化:队长的发言、派单块(SDK 0.3.207 工具名为 Agent)、成员交付以现有
  消息流渲染;首期不做成员内部过程展开,只保证"看得见谁在干什么"。
- **模型选择器恒显示且生效**(用户验收定稿):团队会话模型 = 输入框所选 >
  团队页配置(仅缺省值) > 全局默认。其余聊天能力(附件、上下文)照常。
