...
- `IM / 微信 Clawbot 语音与图片交互`
  ...
  - UI 可验收范围：用户在微信 Clawbot 对话里发送普通语音，Lumos 应识别后进入当前路由会话并回复；用户发送 `/voice on` 或直接说“开启语音模式”后，后续 AI 回复应优先尝试微信语音气泡，若 iLink 拒绝则显示可播放 wav 文件附件；发送 `/voice native off` 后应稳定回到 wav 文件附件；发送 `/voice off` 或说“切回文本模式”后回到文本回复；发送 `/voice status` 可查看当前模式。即使语音生成失败，用户也应收到文本回复而不是无响应。用户要求 AI 生成并发图片时，AI 应调用图片生成工具，微信侧应收到图片附件而不是只看到图片 URL；如果 AI 回复里包含可下载的远程图片 URL，微信侧也应优先收到图片附件，下载失败时才保留 URL 供用户点击。
  - 当前缺口：还没做真实手机微信 / Windows 或 macOS 打包版端到端验收；iLink 原生 voice_item 可能出现接口成功但手机端不展示，只有实机看到微信语音气泡后才能说原生语音出站打通；远程图片下载受 URL 有效期、站点防盗链、图片大小和网络连通性影响，仍需用真实微信对话验证“生成图片 -> 图片附件到达”；本机 Whisper CLI、系统 TTS、长回复切分体验、多人/群聊 peer 级模式隔离也仍需实机确认。
