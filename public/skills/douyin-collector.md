...
- 只拿到标题、作者、封面、链接、视频描述、metadata-only 结果时，不能推断或模拟视频里的发言内容；这些信息不是字幕证据。
- 字幕 / ASR / 总结 / 入库失败或为空时，必须把 `phase` / `error` / `reason` 原样解释给用户，并说明去 `应用 > 抖音采集器` 里更新 Cookie、重试、强制 ASR 或手动入库，不得补写“可能的字幕”。
